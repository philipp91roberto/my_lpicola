...
#ifdef POTENTIAL
  mPot  = (float_kind *)malloc(2*Total_size*sizeof(float_kind));
  cmPot = (complex_kind*)mPot;
#ifdef GONGRID
  PotM = (float_kind *)mPot;
#endif
#endif
#ifdef SINGLE_PRECISION
...
#ifdef POTENTIAL
  pmPot = fftwf_mpi_plan_dft_c2r_3d(Nmesh,Nmesh,Nmesh,cmPot,mPot,MPI_COMM_WORLD,FFTW_ESTIMATE);
#endif
#else
...
#ifdef POTENTIAL
  pmPot = fftw_mpi_plan_dft_c2r_3d(Nmesh,Nmesh,Nmesh,cmPot,mPot,MPI_COMM_WORLD,FFTW_ESTIMATE);
#endif
#endif
#endif
  ...
  for (i=0;i<Local_nx;i++) {
    ...
    for (j=0;j<(unsigned int)(Nmesh/2+1);j++) {
      ...
      for (k=kmin;k<(unsigned int)(Nmesh/2+1);k++) {
        ...
        // cmPot holds now the FFT'ed potential at each mesh point
#ifdef POTENTIAL
        cmPot[ind][1] = -1.0*dens[1]/(Scale*Scale);
        cmPot[ind][0] =      dens[0]/(Scale*Scale);
#endif
...
#ifdef POTENTIAL
  ierr = MPI_Sendrecv(&(mPot[0]),2*alloc_slice*sizeof(float_kind),MPI_BYTE,LeftTask,0,
                      &(mPot[2*last_slice]),2*alloc_slice*sizeof(float_kind),MPI_BYTE,RightTask,0,MPI_COMM_WORLD,&status);
#endif
...
#ifdef POTENTIAL
    P[i].GPot = mPot[(IX*Nmesh+IY)*2*(Nmesh/2+1)+IZ]          *TX*TY*TZ +
                mPot[(IX*Nmesh+IY)*2*(Nmesh/2+1)+IZneigh]     *TX*TY*DZ +
                mPot[(IX*Nmesh+IYneigh)*2*(Nmesh/2+1)+IZ]     *TX*DY*TZ +
                mPot[(IX*Nmesh+IYneigh)*2*(Nmesh/2+1)+IZneigh]*TX*DY*DZ +
                mPot[(IXneigh*Nmesh+IY)*2*(Nmesh/2+1)+IZ]          *DX*TY*TZ +
                mPot[(IXneigh*Nmesh+IY)*2*(Nmesh/2+1)+IZneigh]     *DX*TY*DZ +
                mPot[(IXneigh*Nmesh+IYneigh)*2*(Nmesh/2+1)+IZ]     *DX*DY*TZ +
                mPot[(IXneigh*Nmesh+IYneigh)*2*(Nmesh/2+1)+IZneigh]*DX*DY*DZ;
#endif    
  }
