...
#ifdef POTENTIAL
#if defined (GONGRID) || defined (G4LIGHTCONE)
extern float_kind * PotM;        // The gravitational potential on the grid for the output
#endif
extern float_kind * mPot;        // The gravitational potential grid
extern complex_kind * cmPot;     // Pointer to the complex, FFT'ed gravitational potential grid (use in-place FFT)
extern plan_kind pmPot;          // The plan for the in-place FFT of the gravitational potential grid
#endif
...
#if (defined (GONGRID) || defined (G4LIGHTCONE)) && (defined (GADGET_STYLE) || defined (UNFORMATTED))
extern struct io_header_PotM {
  unsigned int nmesh[6];      // nmesh[1] gives the number of mesh points in the file
  double mass[6];             // mass[1] gives the particle mass
  double time;                // Cosmological scale factor of snapshot
  double redshift;            // redshift of snapshot
  int flag_sfr;               // Flags whether star formation is used
  int flag_feedback;          // Flags whether feedback from star formation is included
  unsigned int nmeshTotal[6]; // nmeshTotal[1] gives the total number of mesh points in the run. If this number exceeds 2^32, the nmeshTotal[2] stores
                              // the result of a division of the particle number by 2^32, while nmeshTotal[1] holds the remainder.
  int flag_cooling;           // Flags whether radiative cooling is included
  int num_files;              // Determines the number of files that are used for a snapshot
  double BoxSize;             // Simulation box size (in code units)
  double Omega0;              // matter density
...
#ifdef POTENTIAL
extern float * iGPot;      // Scalar to hold the gravitational potential before particle initialisation
#endif
...
#ifdef POTENTIAL  
  float GPot;              // The gravitational potential of the particle
#endif
} *P;
#else
...
#ifdef POTENTIAL
extern float_kind * iGPot;    // Scalar to hold the gravitational potential before particle initialisation
#endif
extern struct part_data {
#ifdef PARTICLE_ID
  unsigned long long ID;      // The particle ID
...
#ifdef POTENTIAL
  float_kind GPot;            // The gravitational potential of the particle
#endif
} *P;
#endif
