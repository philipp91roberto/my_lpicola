...
#ifdef MEMORY_MODE
  block = (float *)malloc(bytes = 7*Total_size*sizeof(float_kind)+3*NumPart*sizeof(float));
#else
  ...
  for(n=0; n<NumPartMax; n++) {
    ...
    if (n < NumPart) {
      ...
      if((Delta_Pos[0] > boundary) || (Delta_Pos[1] > boundary) || (Delta_Pos[2] > boundary)) {
        ...
        FatalError((char *)"lightcone.c", 391);
      }
            ...
            if (repflag[coord] == 0) {
              ...
              if (flag) {
                ...
                if (Rpart_new2 > Rcomov_new2) {
...
#ifdef POTENTIAL 
                  // Store the interpolated particle position and velocity, and the gravitational potential.
                  unsigned int ind = 7*(blockmaxlen*repcount+pc[repcount]);
                  ...
                  block[ind + 6] = (float)(potfac*(P[n].GPot));
                  pc[repcount]++;
                  Noutput[coord]++;
#else                  
              ...
              if(pc[repcount] > 0) {
                ...
                if (writeflag[coord] == 0) {
                  ...
                  if(!(fp = fopen(buf, "w"))) {
                    ...
                    FatalError((char *)"lightcone.c", 549);
                  }
                  fflush(stdout);
                  writeflag[coord] = 1;
                } else {
                  if(!(fp = fopen(buf, "a"))) {
                    printf("\nERROR: Can't write in file '%s'.\n\n", buf);
                    FatalError((char *)"lightcone.c", 556);
                  }
                  fflush(stdout);
                }
...
#ifdef UNFORMATTED
#ifdef POTENTIAL
                // write coordinates, velocities and gravitational potential in unformatted binary
                chunk = 7*blockmaxlen*repcount;
                dummy1 = sizeof(pc[repcount]); 
                dummy2 = sizeof(float) * 7 * pc[repcount];
                my_fwrite(&dummy1, sizeof(dummy1), 1, fp);
                my_fwrite(&(pc[repcount]), sizeof(unsigned int), 1, fp);
                my_fwrite(&dummy1, sizeof(dummy1), 1, fp);
                my_fwrite(&dummy2, sizeof(dummy2), 1, fp);
                my_fwrite(&(block[chunk]), sizeof(float), 7 * pc[repcount], fp);
                my_fwrite(&dummy2, sizeof(dummy2), 1, fp);
#else
...
#else
#ifdef POTENTIAL
                // write coordinates, velocities and gravitational potential in ASCII
                for(n=0; n<pc[repcount]; n++) {
                  chunk = 7*(blockmaxlen*repcount+n);
                  fprintf(fp,"%12.6f %12.6f %12.6f %12.6f %12.6f %12.6f %12.6f\n",block[chunk],block[chunk+1],block[chunk+2],block[chunk+3],block[chunk+4],block[chunk+5],block[chunk+6]);
                }
#else
...
void Output_Info_Lightcone(void) {
  ...
  if (ThisTask == 0) {
    ...
    if(!(fp = fopen(buf, "w"))) {
      printf("\nERROR: Can't write in file '%s'.\n\n", buf);
      FatalError((char *)"lightcone.c", 629);
    }
    fflush(stdout);
...
// ===================================================
#ifdef G4LIGHTCONE
void Output_PotM4lightcone(double AI, double A, double AF, double Z) {
  ...
  int iglobal;
  int nprocgroup, groupTask, masterTask, repcount;
  double Rcomov_old  = Light/Hubble*KickStd(AI,1.0);
  double Rcomov_new  = Light/Hubble*KickStd(AF,1.0);
  double Rcomov_old2 = Rcomov_old*Rcomov_old;
  double Rcomov_new2 = Rcomov_new*Rcomov_new;
  ...
  for(groupTask = 0; groupTask < nprocgroup; groupTask++) {
    if (ThisTask == (masterTask + groupTask)) {
      sprintf(buf, "%s/%s_g4lightcone_z%dp%03d.%d", OutputDir, FileBase, (int)Z, (int)rint((Z-(int)Z)*1000), ThisTask);
      if(!(fp = fopen(buf, "w"))) {
        printf("\nERROR: Can't write in file '%s'.\n\n", buf);
        FatalError((char *)"main.c", 707);
      }
      fflush(stdout);
#ifdef UNFORMATTED
      for(k = 0; k < 6; k++) {
      ...
      // write coordinates of the grid between the lightcone radii of the adjacent timesteps
      ...
      for (i = 0; i < Local_nx; i++) {
        for (j = 0; j < Nmesh; j++) {
          for (k = 0, pc = 0; k < Nmesh; k++) {
            ...
            rr = (double)(k*k+iglobal*iglobal+j*j);
            rr2 = pow(rr,2);
            if (rr2 < Rcomov_old2*Rcomov_old2 && rr2 > Rcomov_new2*Rcomov_new2) {
              float x, y, z;
