...
#if defined (LIGHTCONE) && defined (G4LIGHTCONE)
      Output_PotM4lightcone(AI, A, AF, 1.0/A - 1.0);
#endif
      // Step in time
      // ================
      A  = AFF;
...
void Output(double A, double Z, double Dv, double Dv2) {
  ...
  unsigned int n;
  double fac = Hubble/pow(A,1.5);    // This factor transforms the comoving velocity into peculiar velocity / sqrt(a)
  ...
  double potfac = 1.5*Omega * Hubble*Hubble;
  ...
  for(groupTask = 0; groupTask < nprocgroup; groupTask++) {
...
#ifdef MEMORY_MODE
        block = (float *)malloc(bytes = 7*Total_size*sizeof(float_kind));
#else
	...
	printf("%12.6lf, %12.6lf, %12.6lf\n", sumx, sumy, sumz);
        ...
        // adding gravitational potential to output GADGET_STYLE
#ifdef POTENTIAL
        blockpot = (float *)block;
        blockmaxlen = bytes / sizeof(float);

        // write particle potential
        dummy = sizeof(float) * NumPart;
        my_fwrite(&dummy, sizeof(dummy), 1, fp);
        for(n = 0, pc = 0; n < NumPart; n++) {
          blockpot[pc] = (float)(potfac*P[n].GPot);
          pc++;
          if(pc == blockmaxlen) {
            my_fwrite(blockpot, sizeof(float), pc, fp);
            pc = 0;
          }
        }
        if(pc > 0) my_fwrite(blockpot, sizeof(float), pc, fp);
        my_fwrite(&dummy, sizeof(dummy), 1, fp);
#endif
        ...
        free(block);   
#else
        // write coordinates and velocities in ASCII; optionally the gravitational potential
        for(n=0; n<NumPart; n++){
          double P_Vel[3];
...
#ifdef PARTICLE_ID
#ifdef POTENTIAL
          fprintf(fp,"%12llu %15.6f %15.6f %15.6f %15.6f %15.6f %15.6f %15.6f\n",
                      P[n].ID, (float)(lengthfac*P[n].Pos[0]),(float)(lengthfac*P[n].Pos[1]),(float)(lengthfac*P[n].Pos[2]),(float)(velfac*P_Vel[0]),(float)(velfac*P_Vel[1]),(float)(velfac*P_Vel[2]), (float)(potfac*P[n].GPot));          
#endif
          ...
          fprintf(fp,"%15.6f %15.6f %15.6f %15.6f %15.6f %15.6f %15.6f\n",
                      (float)(lengthfac*P[n].Pos[0]),(float)(lengthfac*P[n].Pos[1]),(float)(lengthfac*P[n].Pos[2]),(float)(velfac*P_Vel[0]),(float)(velfac*P_Vel[1]),(float)(velfac*P_Vel[2]), (float)(potfac*P[n].GPot));          
#endif
...
// Output the data of the gravitational potential grid
// ===================================================
#ifdef GONGRID
void Output_PotM(double A, double Z) {
  
  FILE * fp;
  char buf[300];
  unsigned int i, j, k;
  double meshfactor = Box/Nmesh;
  int nprocgroup, groupTask, masterTask;
  double potfac = 1.5*Omega * Hubble*Hubble;

#ifdef GADGET_STYLE
  size_t bytes;
  int pc, dummy, blockmaxlen;
  float * block;
#endif
  ...
  for(groupTask = 0; groupTask < nprocgroup; groupTask++) {
    if (ThisTask == (masterTask + groupTask)) {
      sprintf(buf, "%s/%s_gpot_z%dp%03d.%d", OutputDir, FileBase, (int)Z, (int)rint((Z-(int)Z)*1000), ThisTask);
      if(!(fp = fopen(buf, "w"))) {
        printf("\nERROR: Can't write in file '%s'.\n\n", buf);
        FatalError((char *)"main.c", 933);
      }
      fflush(stdout);
#ifdef GADGET_STYLE
      // Gadget header stuff
      for(k = 0; k < 6; k++) {
        header_PotM.nmesh[k] = 0;
        header_PotM.nmeshTotal[k] = 0;
        header_PotM.mass[k] = 0;
      }
      header_PotM.nmesh[1] = Local_nx*Nmesh*Nmesh;
      header_PotM.nmeshTotal[1] = Nmesh*Nmesh*Nmesh;
      header_PotM.npartTotalHighWord[1] = (unsigned int) (TotNumPart >> 32);
      header_PotM.mass[1] = (3.0*Omega*Hubble*Hubble*Box*Box*Box) / (8.0*PI*G*TotNumPart);
      header_PotM.time = A;
      header_PotM.redshift = Z;

      header_PotM.flag_sfr = 0;
      header_PotM.flag_feedback = 0;
      header_PotM.flag_cooling = 0;
      header_PotM.flag_stellarage = 0;
      header_PotM.flag_metals = 0;
      header_PotM.flag_stellarage = 0;
      header_PotM.flag_metals = 0;
      header_PotM.flag_entropy_instead_u = 0;

      header_PotM.num_files = NTask;
      header_PotM.BoxSize = Box;
      header_PotM.Omega0 = Omega;
      header_PotM.OmegaLambda = OmegaLambda;
      header_PotM.HubbleParam = HubbleParam;

      dummy = sizeof(header_PotM);
      my_fwrite(&dummy, sizeof(dummy), 1, fp);
      my_fwrite(&header_PotM, sizeof(header_PotM), 1, fp);
      my_fwrite(&dummy, sizeof(dummy), 1, fp);

#ifdef MEMORY_MODE
      block = (float *)malloc(bytes = 4*Total_size*sizeof(float_kind));
#else
      block = (float *)malloc(bytes = 10*1024*1024);
#endif
      blockmaxlen = bytes / (4 * sizeof(float));

      // write coordinates and gravitational potential of the gridpoints
      dummy = sizeof(float) * 4 * Local_nx*Nmesh*Nmesh;
      my_fwrite(&dummy, sizeof(dummy), 1, fp);
      for (i = 0; i < Local_nx; i++) {
        for (j = 0; j < Nmesh; j++) {
          for (k = 0, pc = 0; k < Nmesh; k++) {
            float x, y, z;
            x = (float)(meshfactor*i);
            y = (float)(meshfactor*j);
            z = (float)(meshfactor*k);
            block[3 * pc + 0] = x;
            block[3 * pc + 1] = y;
            block[3 * pc + 2] = z;
            block[3 * pc + 3] = (float)(potfac * PotM[(i * Nmesh + j) * 2*(Nmesh/2+1) + k]);
            pc++;
            if(pc == blockmaxlen) {
              my_fwrite(block, sizeof(float), 4 * pc, fp);
              pc = 0;
            }
          }
        }
      }
      if(pc > 0) my_fwrite(block, sizeof(float), 4 * pc, fp);
      my_fwrite(&dummy, sizeof(dummy), 1, fp);

      free(block);

#else
      // write the gravitational potential on the grid and the coordinates of the meshpoints in ASCII
      for (i = 0; i < Local_nx; i++) {
        for (j = 0; j < Nmesh; j++) {
          for (k = 0; k < Nmesh; k++) {
            double x, y, z;
            ...
            y = (float)(meshfactor*j);
            z = (float)(meshfactor*k);
            fprintf(fp, "%15.6f %15.6f %15.6f %15.6f\n", x, y, z, (float)(potfac * PotM[(i * Nmesh + j) * 2*(Nmesh/2+1) + k]));
          }
        }
      }
#endif
